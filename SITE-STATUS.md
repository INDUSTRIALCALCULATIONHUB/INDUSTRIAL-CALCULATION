...
| Master page register | Draft — canonical audit completed; owner decisions required | 328 | Formula-driven workbook at `outputs/master-page-register/MASTER-PAGE-REGISTER.xlsx`; the 2026-08-25 audit found no exact duplicate titles, 10 mapped existing knowledge assets and 5 unmapped legacy knowledge assets. See `MASTER-PAGE-REGISTER-CANONICAL-AUDIT.md`. The workbook remains draft and does not authorize a sitemap, route, page-name or scope change. |
...
| Responsive visual design system | Implemented for current portal pages | 64 | The owner-selected boxed blue-and-teal portal design is implemented on Home, five support pages, four Level 1 domain pages, 34 Level 2 landing pages, 17 Level 3 navigation pages, one Level 4 navigation page, the first final knowledge page and the search page. The shared stylesheet includes a visible mobile search field and a distinct blue selected-navigation state. |
| Sitewide search | Implemented | 96 indexed pages; 64 header controls | Client-side search covers the published page catalogue across knowledge pages, tools, converters and current domain pages. Every visible portal-header search control opens the shared search page; the mobile search field is visible without opening the navigation menu; inactive Fabrication BOM Creator is excluded. |
