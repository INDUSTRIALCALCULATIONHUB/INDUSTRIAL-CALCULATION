...
| Published unit converters | Interface upgrade implemented | 6 of 6 | Area, length, pressure, temperature, volume, and weight/force converters use the approved responsive portal interface and are individually linked from the homepage. |
...
| Responsive visual design system | Implemented for current portal pages | 70 | The owner-selected boxed blue-and-teal portal design is implemented on Home, five support pages, four Level 1 domain pages, 34 Level 2 landing pages, 17 Level 3 navigation pages, one Level 4 navigation page, the first final knowledge page, the search page and all six unit converters. The shared stylesheet includes a visible mobile search field and a distinct blue selected-navigation state. |
| Sitewide search | Implemented | 96 indexed pages; 70 header controls | Client-side search covers the published page catalogue across knowledge pages, tools, converters and current domain pages. Every visible portal-header search control opens the shared search page; the mobile search field is visible without opening the navigation menu; inactive Fabrication BOM Creator is excluded. |
