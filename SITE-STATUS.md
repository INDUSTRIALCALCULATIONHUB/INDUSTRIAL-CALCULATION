...
| Responsive visual design system | Implemented for current portal pages | 45 | The owner-selected boxed blue-and-copper portal design is implemented on Home, five support pages, four Level 1 domain pages, 34 Level 2 landing pages and the search page. The shared stylesheet includes a visible mobile search field. |
| Sitewide search | Implemented | 77 indexed pages; 45 header controls | Client-side search covers the published page catalogue across knowledge pages, tools, converters and current domain pages. Every visible portal-header search control opens the shared search page; the mobile search field is visible without opening the navigation menu; inactive Fabrication BOM Creator is excluded. |
...
| Legal support and AdSense readiness | Partially implemented | 50 | Standalone `disclaimer.html`, shared SVG favicon, valid six-link support navigation and local-link verification completed across 50 public HTML pages. Live consent configuration and final publisher-account review remain required before ads are enabled. |
