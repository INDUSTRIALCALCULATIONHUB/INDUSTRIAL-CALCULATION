...
| Published engineering knowledge pages | Navigation and reading-format upgrade implemented | 15 | All fifteen current root knowledge articles are now surfaced in the homepage knowledge library, use the shared header/search/mobile navigation layer, include an in-page contents list and point to a relevant primary-source baseline. This is an editorial/source upgrade, not a substitute for project-specific engineering sign-off. |
...
| Responsive visual design system | Implemented for current portal pages and retained articles | 125 Level 3 routes + current published pages | The owner-selected boxed blue-and-teal portal design now covers the Level 3 navigation routes and the fifteen retained articles, as well as the previously upgraded homepage, support, domain, Level 2, tool and converter pages. |
| Sitewide search and sitemap | Expanded | 196 sitemap URLs | Client-side search now includes every Level 3 navigation route, and the sitemap includes the four Level 1 pages, 34 Level 2 pages and 125 Level 3 pages. Inactive Fabrication BOM Creator remains excluded. |
...
1. Produce final knowledge pages under the completed Level 3 navigation routes, beginning with the highest-value existing calculator and article cross-links.
2. Establish the formal technical-review and approved-source workflow before marking new knowledge pages ready for publication.
