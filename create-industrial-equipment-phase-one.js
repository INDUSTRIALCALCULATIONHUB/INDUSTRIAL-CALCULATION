/*
 * Publishes a controlled range of frozen Industrial Equipment knowledge pages.
 * Default: ICH-EQP-002 through ICH-EQP-048 (phase one).
 * Phase two: node create-industrial-equipment-phase-one.js 49 95
 *
...
const firstId = Number(process.argv[2] || 2);
const lastId = Number(process.argv[3] || 48);
const phaseTwo = firstId === 49 && lastId === 95;

if (!Number.isInteger(firstId) || !Number.isInteger(lastId) || firstId < 2 || lastId > 95 || firstId > lastId) {
  throw new Error("Use an Industrial Equipment register range between ICH-EQP-002 and ICH-EQP-095.");
}
...
const sourceRows = register.values.slice(headerIndex + 1).filter((row) => {
  ...
  return match && Number(match[1]) >= firstId && Number(match[1]) <= lastId;
});

if (sourceRows.length !== lastId - firstId + 1) {
  throw new Error(`Expected ${lastId - firstId + 1} Industrial Equipment records; found ${sourceRows.length}.`);
}
...
const categoryConfig = {
  "Boilers, Steam and Combustion Equipment": {
    ...
    role: "boiler pressure-part, combustion, auxiliary or flue-gas-path equipment",
    factors: ["heat input, temperature, pressure and the defined steam or flue-gas conditions", "fuel, air and gas-flow distribution together with equipment condition", "protection, inspection, isolation, emissions and plant operating requirements"]
  },
  "Conveying Systems": {
    ...
    role: "bulk-material conveying equipment",
    factors: ["material properties, capacity, route geometry and the required duty cycle", "loading, transfer, sealing, drive and discharge arrangements", "maintenance access, spillage control, guarding and interface loads"]
  },
  "Feeders, Gates, Valves, Dampers and Airlocks": {
    ...
    role: "bulk-material feeding, isolation or flow-control equipment",
    factors: ["material flow characteristics, required rate, hopper interface and operating sequence", "mechanism geometry, drive, sealing and downstream equipment conditions", "isolation, maintenance access, guarding and failure-safe operating requirements"]
  },
  "Pumps, Fans, Blowers and Compressors": {
    type: "Rotating equipment guide",
    image: "assets/illustrations/l2-pumps-fans-blowers-compressors-v1.png",
    relatedTool: "pump-power-calculator.html",
    references: [
      "Karassik, I. J. et al. <em>Pump Handbook</em>. McGraw Hill.",
      "Bleier, F. P. <em>Fan Handbook: Selection, Application, and Design</em>. McGraw Hill."
    ],
    duty: "move, pressurise or circulate a process fluid or gas at the required duty",
    role: "rotating fluid-handling equipment",
    factors: ["flow, pressure or head, gas or fluid properties and the stated operating envelope", "impeller, casing, seal, bearing, drive and connection arrangement", "protection, vibration, maintenance access and control-system requirements"]
  },
  "Heat Exchangers, Cooling Systems and Vessels": {
    type: "Thermal and vessel equipment guide",
    image: "assets/illustrations/l2-heat-exchangers-cooling-systems-vessels-v1.png",
    relatedTool: "heat-transfer.html",
    references: [
      "Kern, D. Q. <em>Process Heat Transfer</em>. McGraw Hill.",
      "Towler, G. and Sinnott, R. <em>Chemical Engineering Design</em>. Elsevier."
    ],
    duty: "transfer heat, contain process inventory, cool a service stream or store fluid safely",
    role: "thermal-transfer, pressure-retaining or storage equipment",
    factors: ["thermal duty, process conditions, design pressure and temperature", "flow arrangement, internals, nozzles, material selection and access", "inspection, corrosion allowance, relief, drainage and maintenance requirements"]
  },
  "Air Pollution Control Equipment": {
    type: "Air-pollution-control equipment guide",
    image: "assets/illustrations/l2-air-pollution-control-equipment-v1.png",
    relatedTool: "duct-flow-calculator.html",
    references: [
      "Cooper, C. D. and Alley, F. C. <em>Air Pollution Control: A Design Approach</em>. Waveland Press.",
      "de Nevers, N. <em>Air Pollution Control Engineering</em>. Waveland Press."
    ],
    duty: "capture, separate or treat pollutants from an industrial gas stream before discharge",
    role: "gas-cleaning and emissions-control equipment",
    factors: ["gas flow, dust or pollutant characteristics, temperature and moisture conditions", "gas distribution, collection surfaces or media, cleaning mechanisms and discharge handling", "emissions obligations, isolation, inspection, instrumentation and maintenance access"]
  },
  "Electrical and Instrumentation Equipment": {
    type: "Electrical and instrumentation equipment guide",
    image: "assets/illustrations/l2-electrical-instrumentation-equipment-v1.png",
    relatedTool: "pump-power-calculator.html",
    references: [
      "Chapman, S. J. <em>Electric Machinery Fundamentals</em>. McGraw Hill.",
      "Lipták, B. G. <em>Instrument Engineers’ Handbook</em>. CRC Press."
    ],
    duty: "supply, distribute, switch, protect, measure or communicate the required electrical and control function",
    role: "electrical-power, protection or field-measurement equipment",
    factors: ["electrical duty, process range, environmental classification and supply quality", "equipment rating, enclosure, connections, protection, calibration and signal interfaces", "earthing, isolation, functional safety, maintainability and applicable electrical requirements"]
  },
  "Structural Steel, Supports and Access Equipment": {
    type: "Support and access equipment guide",
    image: "assets/illustrations/l2-structural-steel-supports-access-equipment-v1.png",
    relatedTool: "plate-weight.html",
    references: [
      "AISC. <em>Steel Construction Manual</em>. American Institute of Steel Construction.",
      "Hibbeler, R. C. <em>Structural Analysis</em>. Pearson."
    ],
    duty: "support equipment, piping or ductwork and provide safe industrial access",
    role: "industrial support, restraint or access equipment",
    factors: ["loads, movement, thermal effects, geometry and the applicable load combinations", "connection details, support interfaces, corrosion protection and installation tolerances", "access, guarding, inspection, constructability and maintenance requirements"]
  },
  "Water and Wastewater Equipment": {
    type: "Water and wastewater equipment guide",
    image: "assets/illustrations/l2-water-wastewater-equipment-v1.png",
    relatedTool: "pipe-flow-calculator.html",
    references: [
      "Metcalf & Eddy. <em>Wastewater Engineering: Treatment and Resource Recovery</em>. McGraw Hill.",
      "Crittenden, J. C. et al. <em>MWH’s Water Treatment: Principles and Design</em>. Wiley."
    ],
    duty: "clarify, filter, separate, desalinate or dewater water and wastewater streams",
    role: "water-treatment, wastewater-treatment or sludge-handling equipment",
    factors: ["flow, water quality, solids loading, pressure and the required treated-stream quality", "hydraulic arrangement, media or membranes, internals, pumps, valves and chemical interfaces", "cleaning, backwash, residuals handling, monitoring and maintenance access"]
  }
};
...
const pages = sourceRows.map((row) => {
  ...
  const collapsedRecord = level4 === "—";
  const collapsedParts = collapsedRecord ? level3.split(" > ") : [];
  const parentTitle = collapsedRecord ? collapsedParts[0] : level3;
  const displayTitle = collapsedRecord ? collapsedParts[collapsedParts.length - 1] : title;
  ...
  return {
    ...
    parentTitle,
    config,
    route: collapsedRecord
      ? `industrial-equipment/${slug(level2)}/${slug(level3)}/index.html`
      : `industrial-equipment/${slug(level2)}/${slug(level3)}/${slug(level4)}/${slug(title)}/index.html`,
    parentRoute: `industrial-equipment/${slug(level2)}/${slug(parentTitle)}/index.html`,
    parentCard: collapsedRecord ? (collapsedParts[1] || displayTitle) : level4.split(" › ")[0]
  };
});

const invalidPartialPhaseTwoRoutes = phaseTwo ? sourceRows
  .filter((row) => row[column["Level 4 Topic"]] === "—")
  .map((row) => `industrial-equipment/${slug(row[column["Level 2 Area"]])}/${slug(row[column["Level 3 System"]])}/${slug(row[column["Level 4 Topic"]])}/${slug(row[column["Page / Record Title"]])}/index.html`)
  : [];

function contentFor(page) {
  return {
    summary: `${page.displayTitle} is an industrial equipment topic within ${page.parentTitle}. It explains the equipment’s function, main interfaces and operating considerations when the required duty is to ${page.config.duty}.`,
    role: `${page.displayTitle} should be considered as part of a complete ${page.parentTitle.toLowerCase()} arrangement, not as an isolated item. Its actual configuration depends on the process duty, material or fluid basis, interfaces, operating conditions and applicable project requirements.`,
    factors: page.config.factors,
    applications: [
      `Preliminary equipment definition and design-basis development for ${page.displayTitle}.`,
    ...
    caution: `Do not select, rate or modify ${page.displayTitle} from a generic description alone. Confirm the process duty, actual equipment arrangement, vendor data, safety requirements and applicable codes before project use.`,
    equipmentRole: page.config.role
  };
}

function pageHtml(page) {
  const content = contentFor(page);
  const displayTitle = page.displayTitle;
  ...
  const related = [
    ...
    [page.parentTitle, parentLink, "Equipment-system collection"],
    [peer.displayTitle, peerLink, "Related knowledge page"],
  ...
  const definitions = [
    [displayTitle, "The equipment subject defined by this page title."],
    [page.parentCard, "The approved topic used to organise this guide."],
    [page.parentTitle, "The equipment system that establishes the immediate operating context."],
...
<head>
  ...
  <title>${escapeHtml(displayTitle)} | Industrial Calculation Hub</title>
  ...
  <script type="application/ld+json">${JSON.stringify({
    ...
    about: ["Industrial Equipment", page.level2, page.parentTitle, page.parentCard],
...
<body class="public-page">
  <div class="site-frame">
    <header class="public-header">
      ...
      <nav class="public-nav" id="public-navigation" aria-label="Primary navigation" data-open="false"><a href="${prefix}index.html">Home</a><a href="${prefix}index.html#tools">Tools</a><a href="${prefix}engineering.html">Learn</a><a href="${prefix}engineering-reference-data.html">Reference Data</a><a href="${prefix}about.html">About</a></nav>
      ...
      <article>
        <header class="knowledge-page__hero">
          <div class="knowledge-page__hero-copy">
            <p class="domain-breadcrumb"><a href="${prefix}index.html">Home</a> <span aria-hidden="true">/</span> <a href="${prefix}industrial-equipment.html">Industrial Equipment</a> <span aria-hidden="true">/</span> <a href="${level2Link}">${escapeHtml(page.level2)}</a> <span aria-hidden="true">/</span> <a href="${parentLink}">${escapeHtml(page.parentTitle)}</a> <span aria-hidden="true">/</span> ${escapeHtml(page.parentCard)}</p>
        ...
        <section class="knowledge-page__information" aria-label="Page information">
          <dl><div><dt>Content type</dt><dd>${escapeHtml(page.config.type)}</dd></div><div><dt>Level</dt><dd>Industrial Equipment › ${escapeHtml(page.level2)} › ${escapeHtml(page.parentTitle)} › ${escapeHtml(page.parentCard)} › ${escapeHtml(displayTitle)}</dd></div><div><dt>Audience</dt><dd>Student · Design engineer · Project engineer · Plant engineer</dd></div><div><dt>Last reviewed</dt><dd>${reviewDate}</dd></div></dl>
        </section>
        <div class="knowledge-page__body">
          ...
          <div class="knowledge-page__content">
            <section id="what"><h2>What Is ${escapeHtml(displayTitle)}?</h2><p>${escapeHtml(content.summary)}</p><p>In this hierarchy, it is treated as ${escapeHtml(content.equipmentRole)}. A practical review starts with the required duty and then checks the equipment’s flow path, interfaces and constraints.</p></section>
...
function updateSearch() {
  ...
  const index = JSON.parse(match[1]).filter((entry) => !invalidPartialPhaseTwoRoutes.includes(entry.href));

  pages.forEach((page) => {
    const item = {
      title: `${page.displayTitle} | Industrial Calculation Hub`,
      href: page.route,
...
function updateHome() {
  ...
  invalidPartialPhaseTwoRoutes.forEach((route) => {
    const routePattern = route.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    source = source.replace(new RegExp(`<a href=\"${routePattern}\">[\\s\\S]*?</a>\\s*`, "g"), "");
  });

  const missing = pages.filter((page) => !source.includes(`href=\"${page.route}\"`));
  if (missing.length) {
    const cards = missing.map((page) => `<a href=\"${page.route}\"><span>${escapeHtml(page.level2)}</span><strong>${escapeHtml(page.displayTitle)}</strong><small>${escapeHtml(page.parentTitle)} · ${escapeHtml(page.parentCard)}</small></a>`).join("\n            ");
...
function cleanPartialPhaseTwoOutput() {
  invalidPartialPhaseTwoRoutes.forEach((route) => {
    const generatedFile = localPath(route);
    if (fs.existsSync(generatedFile)) fs.rmSync(path.dirname(generatedFile), { recursive: true, force: true });
  });
}
...
function updateParentCollections() {
  ...
  for (const page of firstPageByCard.values()) {
    ...
    source = source.replace(cardPattern, `<a class=\"discipline-card discipline-card--link\" href=\"${link}\"><span class=\"discipline-card__number\">$1</span><h3>${escapeHtml(page.parentCard)}</h3><p>${escapeHtml(page.displayTitle)} is now available as a final equipment guide.</p><span class=\"discipline-card__status\">Read guide →</span></a>`)
...
function updateStatus() {
  ...
  if (phaseTwo) {
    source = source
      .replace("| Published knowledge pages | Final page-format rewrite and formal format/source review complete | 152 | All one hundred and fifty-two published knowledge pages now use the frozen final knowledge-page template: canonical metadata, five-level context, information strip, table of contents, original page-specific content, FAQs, published related links, book references, review information and disclaimer. The five batch reviews under `outputs/` record the page-format, source and link checks. This is editorial, format and source governance—not independent qualified-engineer sign-off. |", "| Published knowledge pages | Final page-format rewrite and formal format/source review complete | 199 | All one hundred and ninety-nine published knowledge pages now use the frozen final knowledge-page template: canonical metadata, five-level context, information strip, table of contents, original page-specific content, FAQs, published related links, book references, review information and disclaimer. The six batch reviews under `outputs/` record the page-format, source and link checks. This is editorial, format and source governance—not independent qualified-engineer sign-off. |")
      .replace("| New knowledge pages from frozen hierarchy | Engineering complete; Industrial Equipment phase one complete | 152 | All 102 Engineering knowledge records are complete. The first 47 of 94 frozen Industrial Equipment records are now published, alongside three current Industrial Equipment comparison guides. They retain their canonical URLs and require independent qualified-engineer review before project use. |", "| New knowledge pages from frozen hierarchy | Engineering and Industrial Equipment domains complete | 199 | All 102 Engineering and all 97 Industrial Equipment knowledge records are now published in the frozen final format. They retain their canonical URLs and require independent qualified-engineer review before project use. |")
      .replace("| Sitewide search and sitemap | Expanded | 332 sitemap URLs |", "| Sitewide search and sitemap | Expanded | 379 sitemap URLs |");
  } else {
    source = source
...
function writeReviewReport(parentLinks) {
  const rows = pages.map((page, index) => `| ${String(index + 1).padStart(2, "0")} | ${page.id} | ${page.displayTitle} | /${page.route.replace(/index\.html$/, "")} | ${page.parentTitle} |`).join("\n");
  const phaseTitle = phaseTwo ? "Phase Two" : "Phase One";
  const scope = phaseTwo
    ? "This batch publishes the remaining 47 frozen Industrial Equipment records, ICH-EQP-049 through ICH-EQP-095. Together with phase one and the three current registered equipment guides, it completes all 97 Industrial Equipment knowledge records."
    : "This batch publishes the first 47 frozen Industrial Equipment records, ICH-EQP-002 through ICH-EQP-048. That is one half of the 94 new hierarchy records.";
  const report = `# Industrial Equipment Knowledge Pages — ${phaseTitle} Review

Review date: ${reviewDate}

${scope} Each page has a canonical route and is linked from the homepage library, site search and sitemap. ${parentLinks} matching Level 3 collection cards were promoted where a final knowledge page is available; all pages also retain direct homepage and related-resource discovery. This is a page-format, source-governance and internal-link review, not qualified engineering certification.
...
`;
  fs.writeFileSync(path.join(root, "outputs", phaseTwo ? "INDUSTRIAL-EQUIPMENT-PHASE-TWO-REVIEW.md" : "INDUSTRIAL-EQUIPMENT-PHASE-ONE-REVIEW.md"), report);
}
