(function () {
  ...
  function renderModel() {
    ...
    const view = (cx, baseY, centreBottom, insideBottom, offset, title) => {
      ...
      const ticks = (x1, y1, x2, y2) => `<path d="M${x1 - 5},${y1 - 5} L${x1 + 5},${y1 + 5} M${x2 - 5},${y2 - 5} L${x2 + 5},${y2 + 5}" class="dimension-line"/>`;
  ...
  function exportDxf() {
    ...
    const tick=(p,size=textHeight*.45)=>line({x:p.x-size,y:p.y-size},{x:p.x+size,y:p.y+size},"DIMENSIONS");
    const dimension=(a,q,label,rotation=0)=>{line(a,q,"DIMENSIONS");tick(a);tick(q);text({x:(a.x+q.x)/2,y:(a.y+q.y)/2+textHeight*.4},label,textHeight,rotation);};
    const inputView=(cx,bottomSize,insideSize,offset,title)=>{
      const bottomL={x:cx-bottomSize/2,y:inputBaseY},bottomR={x:cx+bottomSize/2,y:inputBaseY},topC=cx+offset,topL={x:topC-c.diameter/2,y:inputBaseY+c.height},topR={x:topC+c.diameter/2,y:inputBaseY+c.height},neck=textHeight*3;
      line(bottomL,topL,"INPUT_GEOMETRY");line(topL,topR,"INPUT_GEOMETRY");line(topR,bottomR,"INPUT_GEOMETRY");line(bottomR,bottomL,"INPUT_GEOMETRY");line(bottomL,{x:topC,y:topL.y},"INPUT_GUIDES");line(bottomR,{x:topC,y:topL.y},"INPUT_GUIDES");line(topL,{x:topL.x,y:topL.y+neck},"INPUT_GEOMETRY");line(topR,{x:topR.x,y:topR.y+neck},"INPUT_GEOMETRY");line({x:topL.x,y:topL.y+neck},{x:topR.x,y:topR.y+neck},"INPUT_GEOMETRY");line({x:cx,y:inputBaseY},{x:topC,y:topL.y+neck},"CENTER_LINES");
      dimension({x:bottomL.x,y:inputBaseY-textHeight*2},{x:bottomR.x,y:inputBaseY-textHeight*2},`${insideSize} I/S`);dimension({x:topL.x,y:topL.y+neck+textHeight*2},{x:topR.x,y:topR.y+neck+textHeight*2},`DIA ${j.diameter} DUCT I/S`);dimension({x:bottomL.x-textHeight*2,y:inputBaseY},{x:bottomL.x-textHeight*2,y:topL.y},`${j.height}`,90);dimension({x:cx,y:topL.y-textHeight*2},{x:topC,y:topL.y-textHeight*2},`${title==="ELEVATION"?"OFFSET X":"OFFSET Y"} = ${offset}`);text({x:cx-bottomSize/4,y:inputBaseY-textHeight*6},title,textHeight*1.2,0,"NOTES");
    };
    ...
    line({x:planC.x-c.width/2,y:planC.y},{x:planC.x+c.width/2,y:planC.y},"CENTER_LINES");line({x:planC.x,y:planC.y-c.depth/2},{x:planC.x,y:planC.y+c.depth/2},"CENTER_LINES");dimension(planC,{x:planCircle.x,y:planC.y},`OFFSET X = ${c.offsetX}`);dimension({x:planCircle.x,y:planC.y},planCircle,`OFFSET Y = ${c.offsetY}`,90);text({x:planC.x-c.width/5,y:planC.y-c.depth/2-textHeight*4},"PLAN",textHeight*1.2,0,"NOTES");
