(function () {
  ...
  function completeSvg(complete, width, height, margin) {
    ...
    const segStart=map(complete.upper[0]),segEnd=map(complete.upper[1]),segMid={x:(segStart.x+segEnd.x)/2,y:(segStart.y+segEnd.y)/2},mappedBoundary=complete.boundary.map(map),drawingCentre=mappedBoundary.reduce((s,p)=>({x:s.x+p.x/mappedBoundary.length,y:s.y+p.y/mappedBoundary.length}),{x:0,y:0}),sdx=segEnd.x-segStart.x,sdy=segEnd.y-segStart.y,slen=Math.max(1,Math.hypot(sdx,sdy)),normal={x:-sdy/slen,y:sdx/slen},candidateA={x:segMid.x+normal.x*25,y:segMid.y+normal.y*25},candidateB={x:segMid.x-normal.x*25,y:segMid.y-normal.y*25},distanceFromCentre=p=>Math.hypot(p.x-drawingCentre.x,p.y-drawingCentre.y),segmentTextPoint=distanceFromCentre(candidateA)>distanceFromCentre(candidateB)?candidateA:candidateB,segmentLabel=`${TransitionGeometry.distance2(complete.upper[0], complete.upper[1]).toFixed(2)} TYP`;
    let segmentRotation=lineAngle(segStart,segEnd);if(segmentRotation>90||segmentRotation< -90)segmentRotation+=180;
    const typicalSegment=`<text x="${segmentTextPoint.x}" y="${segmentTextPoint.y}" text-anchor="middle" transform="rotate(${segmentRotation} ${segmentTextPoint.x} ${segmentTextPoint.y})" class="segment-callout">${segmentLabel}</text>`;
  ...
  function exportDxf() {
    ...
    const dxfSegStart=model.complete.upper[0],dxfSegEnd=model.complete.upper[1],dxfSegMid={x:(dxfSegStart.x+dxfSegEnd.x)/2,y:(dxfSegStart.y+dxfSegEnd.y)/2},dxfCentre=model.complete.boundary.reduce((s,p)=>({x:s.x+p.x/model.complete.boundary.length,y:s.y+p.y/model.complete.boundary.length}),{x:0,y:0}),dxfDx=dxfSegEnd.x-dxfSegStart.x,dxfDy=dxfSegEnd.y-dxfSegStart.y,dxfLen=Math.max(1,Math.hypot(dxfDx,dxfDy)),dxfNormal={x:-dxfDy/dxfLen,y:dxfDx/dxfLen},dxfCandidateA={x:dxfSegMid.x+dxfNormal.x*textHeight*2.2,y:dxfSegMid.y+dxfNormal.y*textHeight*2.2},dxfCandidateB={x:dxfSegMid.x-dxfNormal.x*textHeight*2.2,y:dxfSegMid.y-dxfNormal.y*textHeight*2.2},dxfDistance=p=>Math.hypot(p.x-dxfCentre.x,p.y-dxfCentre.y),segmentNote=dxfDistance(dxfCandidateA)>dxfDistance(dxfCandidateB)?dxfCandidateA:dxfCandidateB;let dxfSegmentRotation=lineAngle(dxfSegStart,dxfSegEnd);if(dxfSegmentRotation>90||dxfSegmentRotation< -90)dxfSegmentRotation+=180;text(segmentNote,`${TransitionGeometry.distance2(dxfSegStart,dxfSegEnd).toFixed(2)} TYP`,textHeight*.85,dxfSegmentRotation);
