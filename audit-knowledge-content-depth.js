...
const markdown = [
  ...
  '',
  'A full content upgrade is required when a page has fewer than 1,500 visible words or fewer than 6 FAQs. Targeted enrichment is required at 1,500–1,999 words or 6–7 FAQs. The working baseline is 2,000+ visible words and 8+ genuinely useful FAQs, with content tailored to the topic rather than padded.',
  '',
...
const fluidUpdateReport = [
  ...
  '',
  '| Topic | Updated guide | Visible words | FAQs | In-article visual | Status |',
  '| --- | --- | ---: | ---: | --- | --- |',
  ...fluidRows.map((row) => `| ${row.topic} | ${row.title.replace(/\\|/g, '\\\\|')} | ${row.words} | ${row.faqs} | ${row.visual} | Updated |`),
  '',
  `**Total updated guides:** ${fluidRows.length}. Every guide meets the working baseline of 2,000+ visible words and 8+ useful FAQs.`,
  '',
